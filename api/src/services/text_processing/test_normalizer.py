...
Tests run standalone: pytest deploy/desktop/kokoro/test_normalizer.py -v
No Docker or Kokoro dependencies required — we import handle_markdown directly.
"""
...
# ---------------------------------------------------------------------------
# Shim: fake Kokoro's third-party and internal dependencies so we can import
# normalizer.py without the full Kokoro environment.  We only need
# handle_markdown(), which uses nothing beyond the stdlib `re` module.
# ---------------------------------------------------------------------------

# Third-party libs used at module level in normalizer.py
for _dep in ("inflect", "numpy", "numpy.number", "torch"):
    if _dep not in sys.modules:
        sys.modules[_dep] = MagicMock()

# Internal Kokoro package hierarchy for the relative import:
#   from ...structures.schemas import NormalizationOptions
_fake_schemas = types.ModuleType("api.src.structures.schemas")
...
_fake_schemas.NormalizationOptions = _StubNormalizationOptions  # type: ignore[attr-defined]
...
sys.modules["api.src.structures.schemas"] = _fake_schemas

# Wire up sub-module attributes so dotted access works
sys.modules["api"].src = sys.modules["api.src"]  # type: ignore[attr-defined]
sys.modules["api.src"].services = sys.modules["api.src.services"]  # type: ignore[attr-defined]
sys.modules["api.src"].structures = sys.modules["api.src.structures"]  # type: ignore[attr-defined]
sys.modules["api.src.services"].text_processing = sys.modules["api.src.services.text_processing"]  # type: ignore[attr-defined]
sys.modules["api.src.structures"].schemas = _fake_schemas  # type: ignore[attr-defined]

# Import normalizer with correct __package__ for relative imports
import importlib.util

_spec = importlib.util.spec_from_file_location(
    "api.src.services.text_processing.normalizer",
    Path(__file__).resolve().parent / "normalizer.py",
    submodule_search_locations=[],
)
_mod = importlib.util.module_from_spec(_spec)  # type: ignore[arg-type]
_mod.__package__ = "api.src.services.text_processing"  # type: ignore[attr-defined]
sys.modules[_spec.name] = _mod  # type: ignore[union-attr]
_spec.loader.exec_module(_mod)  # type: ignore[union-attr]

handle_markdown = _mod.handle_markdown  # type: ignore[union-attr]
